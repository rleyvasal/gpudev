...
def _data(name: str):
    ...
    if c["kind"] == "remote":
        host, opts = _ssh_cfg()
        remote_cmd = "cat -- " + shlex.quote(c["src"])
...
def _ensure_server():
    ...
    # Solveit preview uses fixed port 8000; clear any previous local viewer server.
...
def point_cloud(src, *, stride=5, size=0.06, color="height", icol=3, remote=True, name=None):
    """Visualise a point cloud and return the inline viewer.

    src    : remote path default, local path with remote=False, or numpy (N,K) array.
...
try:
    ...
    def pointcloud_var(line):
        ...
        if not parts:
            raise ValueError(
                "usage: %pointcloud_var <expr> [sub=N] [color=...] "
                "[size=F] [icol=N] [name=...]"
            )

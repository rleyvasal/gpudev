...
def install_core(*, quiet: bool = False) -> bool:
    ...
    global _exec_mgr, _mojo_mgr, ROUTER, PY_BACKEND, MOJO_BACKEND
    ...
    if not quiet:
        ...
        print("  %gpum %restart_mojo %mojo_* %bench   (Mojo)")
        print("  remote_run_(code)  register_local_magic('%name')")
        print("  Optional: install_pcviz() / install_sslive()  (see CRAFT.py loader)")
        if not magics_ok:
